...
class CreditScoreServiceTest {
    ...
    void getCreditScore_withSsafyLink() {
        ...
        assertThat(response.getTotalAsset()).isEqualTo(12_345_678L);
        assertThat(response.getTotalDebt()).isEqualTo(2_000_000L);
        assertThat(response.getNetAsset()).isEqualTo(8_000_000L);
    }

    @DisplayName("SSAFY 미연동 사용자는 로컬 요약 자산 정보를 반환한다")
    @Test
    void getCreditScore_withoutSsafyLink() {
        // given
        final Long userId = 1L;
        final CreditScore creditScore = CreditScore.of(280, 230, 110, 80, 70);
        final UserFinancialSummary summary = summary(userId, 10_000_000, 1_500_000, 8_500_000);
        ...
        assertThat(response.getTotalAsset()).isEqualTo(10_000_000L);
        assertThat(response.getTotalDebt()).isEqualTo(1_500_000L);
        assertThat(response.getNetAsset()).isEqualTo(8_500_000L);
    ...
    private UserFinancialSummary summary(
        ...
        summary.refresh(totalAssetAmount, totalDebtAmount, netAssetAmount, 5_000_000, 3_000_000, 2_000_000);
        return summary;
    }
}
