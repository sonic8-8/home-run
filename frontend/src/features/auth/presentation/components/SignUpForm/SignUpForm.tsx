...
const TERMS_CONTENT = `홈런 서비스 이용약관

제1조 (목적)
이 약관은 홈런(이하 "회사"라 함)이 제공하는 서비스의 이용과 관련하여 회사와 이용자 간의 권리, 의무 및 책임사항, 기타 필요한 사항을 규정함을 목적으로 합니다.
...
2. 회사는 다음 각 호에 해당하는 신청에 대해서는 승낙을 하지 않거나 사후에 이용계약을 해지할 수 있습니다.
   회사는 이용자 청 요건을 충족하는 모든 이용자의 신청을 승낙합니다.
...
3. 회원의 아이디 및 비밀번호가 도용되거나 제3자가 사용하고 있음을 인지한 경우에는 이를 즉시 회사에 통지하고 회사의 안내에 따라야 합니다.
...
1. 이용자는 다음 행위를 하여서는 안 됩니다.
   ...
   5) 회사의 기타 제3자의 저작권 등 지식재산권에 대한 침해
   6) 회사 및 기타 제3자의 명예를 손상시키거나 업무를 방해하는 행위
   7) 외설 또는 폭력적인 메시지, 화상, 음성, 기타 공서양속에 반하는 정보를 서비스에 공개 또는 게시하는 행위
   8) 기타 불법적이거나 부당한 행위
2. 이용자는 관계법령, 이 약관의 규정, 이용자내 및 서비스와 관련하여 공지한 주의사항, 회사가 통지하는 사항 등을 준수하여야 하며, 기타 회사의 업무에 방해되는 행위를 하여서는 안 됩니다.

제10조 (서비스의 제공 및 변경)
1. 회사는 다음과 같은 서비스를 제공합니다.
   마이데이터, 카드 이용내역, 시드머니 저축 관리, 적출 서비스
2. 회사는 상당한 이유가 있는 경우의 이상의, 기술 상 변화의 필요에 따라 제공하고 있는 서비스를 변경할 수 있습니다.
3. 회사는 이용자에게 서비스를 제공함에 있어 관련 법령, 약관, 이용의 성 및 공지사항 등에서 정한 바에 따라 무료로 서비스를 제공합니다.

제11조 (서비스의 중단)
1. 회사는 컴퓨터 등 정보통신 설비의 보수점검, 교체 및 고장, 통신의 두절 등의 사유가 발생한 경우에는 서비스의 제공을 일시적으로 중단할 수 있습니다.
...
4. 회사가 회원자격을 상실시키는 경우에는 회원등록을 말소합니다. 이 경우 회원에게 이를 통지하고, 회원등록 말소 전 최소한 30일 이상의 기간을 정하여 소명할 기회를 부여합니다.

제13조 (정보의 제공 및 광고의 게재)
회사는 회원에게 서비스 이용에 필요한 정보를 공지사항이나 전자우편 등의 방법으로 제공할 수 있습니다. 다만, 회사는 회원이 동의하지 않는 한 영리목적의 광고성 정보를 제공하지 않습니다.

제14조 (서비스 이용시간)
1. 서비스 이용은 회사의 업무상 또는 기술상 특별한 지장이 없는 한 연중무휴, 1일 24시간 이상을 원칙으로 합니다.
2. 회사는 서비스를 일정 범위로 분할하여 각 범위별로 이용가능 시간을 별도로 정할 수 있습니다. 이 경우 그 내용을 사전에 공지합니다.

제15조 (서비스 이용 제한)
1. 회사는 전쟁, 재변, 천재지변 또는 이에 준하는 국가비상사태가 발생하거나 발생할 우려가 있는 경우와 전기통신사업법에 의한 기간통신 사업자가 전기통신 서비스를 중지하는 등 기타 불가항력적 사유가 있는 경우에는 서비스의 전부 또는 일부를 제한하거나 중지할 수 있습니다.
...
제17조 (준거법 및 재판관할)
1. 회사와 회원 간에 제기되는 소송은 대한민국법을 준거법으로 합니다.
2. 회사와 회원 간에 발생한 분쟁에 관한 소송은 이용자 주소지 관할 법원을 관할로 합니다.

제18조 (기타)
1. 이 약관에 명시되지 않은 사항은 관련 법령의 규정에 따릅니다.
2. 회사는 필요한 경우 특정 서비스에 관하여 별도의 이용약관 및 정책을 둘 수 있으며, 해당 내용이 이 약관과 충돌할 경우에는 별도의 이용약관 및 정책이 우선하여 적용됩니다.

부칙
1. 이 약관은 2026-03-24부터 시행합니다.`;
        ...
        <div className={styles.modalOverlay} onClick={() => setShowTerms(false)}>
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.modalBody}>
              <pre className={styles.termsContent}>{TERMS_CONTENT}</pre>
            </div>
